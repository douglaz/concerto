...
use crate::common::{
    build_and_load_daemon_image, configure_coredns, connect_to_k8s, create_storage_classes,
    deploy_daemon_rbac, deploy_postgres_if_needed, install_nginx_ingress_controller,
    label_nodes_with_zone, load_fedimint_images, wait_for_postgres, EnvConf,
};
...
async fn run_integration_tests(run_args: RunArgs) -> anyhow::Result<()> {
    ...
    k3d_test::test_federation_status_transitions(
        &kube_client,
        &env_conf,
        &image_tag,
        &run_args.pg,
